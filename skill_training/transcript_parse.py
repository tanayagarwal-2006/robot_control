...
in "Move end-effector to position". Always include the keys "x","y","z","rx","ry","rz","gripper_position" regardless of whatever
adjustment command is given by the user.
...
If the user is specifically creating a sequence, do not put any of commands under any other key in the dict, only keep them in 
'Save sequence' key.
...
If the user commands to enter the skills training mode, output this as True, otherwise keep it as None.

- Execute learned skill
If the user says to perform a previously trained skill (like pouring water or making popcorn), return the skill name in this field.  
Use lowercase and underscores between words.
...
Input: "Can you make some popcorn for me?"  
→ {{"Execute learned skill": "make_popcorn"}}

Input: "Turn off the kitchen light"  
→ {{"Execute learned skill": "turn_off_light"}}

Input: "Make me a drink"  
→ {{"Execute learned skill": "make_drink"}}
